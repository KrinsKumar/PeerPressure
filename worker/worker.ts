...
class Worker {
  ...
  private setupUiSubmission() {
    this.expressApp.get("/filesubmission", (req: Request, res: Response) => {
      // get the file from the body
      const file = req.body.file;
      // convert into buffer
      const buffer = Buffer.from(file);
      // upload the file
      this.uploadFile(buffer.toString(), true, file.name);
    });
  }
  ...
  cli() {
    ...
    console.log(
      "9: upload ./examples/example.txt - Upload a specific example file"
    );
